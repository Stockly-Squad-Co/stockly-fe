  ...
  return (
    <div className="space-y-1 relative min-w-fit">
      ...
      <div
        {...containerProps}
        className={cn(
          `bg-white dark:bg-white/10 duration-200 hover:bg-gray-100 dark:hover:bg-transparent w-full p-[9px] border dark:border-white/10 rounded-lg select-none flex items-center justify-between gap-2 ${
      ...
      >
        {loading ? (
          ...
          <p className="text-[.8rem] w-full capitalize">{pickedData.label}</p>
        )}
        <FaChevronUp
          size={14}
    ...
    >
      <>
        {stateOptions.map((opt, id) => (
          <div
            key={id}
            className="p-2 capitalize duration-200 cursor-pointer dark:bg-secondary-900 bg-white dark:bg-[#1a1a1a] text-sm hover:bg-gray-100 dark:hover:bg-[#383838]"
